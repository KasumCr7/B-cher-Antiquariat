...
$where = ['1=1'];
$params = [];
if ($search) { $where[] = '(b.Title LIKE ? OR b.autor LIKE ?)'; $params[] = "%$search%"; $params[] = "%$search%"; }
if ($filterKat) { $where[] = 'b.kategorie = ?'; $params[] = $filterKat; }
$whereSQL = implode(' AND ', $where);

$stmt = $db->prepare("SELECT b.id, b.katalog, b.nummer, b.Title AS titel, b.autor, b.zustand, b.verkauft, k.kategorie FROM buecher b LEFT JOIN kategorien k ON k.id = b.kategorie WHERE $whereSQL ORDER BY b.Title LIMIT 200");
$stmt->execute($params);
$books = $stmt->fetchAll();
                ...
                <form method="post" action="books.php">
                    ...
                    <div class="form-row form-row-2">
                        <div class="form-group">
                            ...
                            <input type="number" name="katalog" class="form-control" value="<?= htmlspecialchars($editBook['katalog'] ?? '') ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Nummer *</label>
                            <input type="number" name="nummer" class="form-control" value="<?= htmlspecialchars($editBook['nummer'] ?? '') ?>" required>
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Titel *</label>
                        <input type="text" name="titel" class="form-control" value="<?= htmlspecialchars($editBook['Title'] ?? '') ?>" required>
                    </div>

                    <div class="form-row form-row-2">
                        <div class="form-group">
                            <label>Autor *</label>
                            <input type="text" name="autor" class="form-control" value="<?= htmlspecialchars($editBook['autor'] ?? '') ?>" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="zustand" class="form-control">
                                <?php foreach (['S' => 'Sehr gut', 'G' => 'Gut', 'M' => 'Mittel', 'S-' => 'Schlecht'] as $val => $label): ?>
                                    <option value="<?= $val ?>" <?= ($editBook['zustand'] ?? '') === $val ? 'selected' : '' ?>><?= $label ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group" style="display:flex;align-items:flex-end;">
                            <div class="checkbox-group">
                                <input type="checkbox" id="verkauft" name="verkauft" value="1" <?= !empty($editBook['verkauft']) ? 'checked' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="beschreibung" class="form-control"><?= htmlspecialchars($editBook['Beschreibung'] ?? '') ?></textarea>
                    </div>
        ...
        <?php else: ?>
            ...
            <div class="admin-header">
                <div>
                    ...
                    <p>Alle Bücher im Bestand (<?= count($books) ?>)</p>
                </div>
            ...
            <form method="get" action="books.php" class="admin-filter">
                <input type="text" name="search" class="form-control" placeholder="Titel oder Autor suchen..." value="<?= htmlspecialchars($search) ?>">
            ...
            <div class="table-wrapper">
                <table>
                    ...
                    <tbody>
                        <?php if (empty($books)): ?>
                            <tr><td colspan="8" class="text-center text-muted" style="padding:2rem;">Keine Bücher gefunden</td></tr>
                        <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="td-actions">
                                            <a href="books.php?action=edit&id=<?= $b['id'] ?>" class="btn btn-ghost btn-sm" title="Bearbeiten">✏️</a>
                                            <form method="post" action="books.php" onsubmit="return confirm('Buch &quot;<?= htmlspecialchars(addslashes($b['titel'] ?? '')) ?>&quot; wirklich löschen?');" style="display:inline;">
